    ...
    if isinstance(event, MessageEvent):
        ...
        if user_info.part == 0:
            if event.message.text == "我到駐警隊了":
                reply = [] #一次可傳多對話，至多五句
                reply.append(TextSendMessage("警衛: 同學請問有什麼事嗎?"))
                reply.append(TextSendMessage("我: 我的腳踏車不見了，想請你們幫忙調閱監視器!"))
                reply.append(TextSendMessage("警衛(一邊查監視器)：喔對了同學，最近一直有人來抱怨這附近有一棵「看起來很危險，會壓到兩個人」的樹，叫駐警隊處理，雖然維護校園安全是我們的工作範圍啦，可是講這麼模糊誰知道阿？你可以幫我調查一下是怎麼回事嗎？"))
                line_bot_api.reply_message(event.reply_token, reply)
                User_Info.objects.filter(uid=event.source.user_id).update(part=1)
            else:
            ...
            if event.message.text == "朴樹":
                reply = [] #一次可傳多對話，至多五句
                reply.append(TextSendMessage("警衛：太感謝你了！剛好我這邊也調到影像了，你來看看是不是你的車子"))
                #放監視器圖片
                reply.append(TextSendMessage("我：原來舟山路是案發現場！"))
    ...
    if isinstance(event, MessageEvent):
        ...
        if user_info.part == 0:
            if event.message.text == "我到案發現場了":
                reply = [] #一次可傳多對話，至多五句
                reply.append(TextSendMessage("大笨鳥的靈魂：那裏有人在拍照！我們去問他有沒有看到是誰壓到我的！"))
                reply.append(TextSendMessage("學生:明明這裡都有動物穿越減速慢行的路牌，那個人車還是騎得很快，結果撞死大笨鳥後就跑了，這已經不是第一次了。"))
                reply.append(TextSendMessage("我：那你有看到犯人壓到大笨鳥後去了哪裡嗎？"))
                reply.append(TextSendMessage("學生:好像有看到往哪裡去......，不過我在進行科普閱讀的推廣與實踐，你看起來是個聰明人，我把答案包裝成謎題，你自己去找尋答案吧，相信難不倒你。"))
            ...
            if event.message.text == "圖書館" or event.message.text == "總圖" or event.message.text == "台大圖書館":
                reply = [] #一次可傳多對話，至多五句
                reply.append(TextSendMessage("大笨鳥的靈魂：原來往圖書館去了！！我們趕快追過去"))
    ...
    if isinstance(event, MessageEvent):
        ...
        if user_info.part == 0:
            if event.message.text == "我到圖書館了":
                ...
                line_bot_api.reply_message(event.reply_token, TextSendMessage("大笨鳥的靈魂：我們去圖書館! 請輸入「我到圖書館了」"))
        elif user_info.part == 1:
    ...
    if isinstance(event, MessageEvent):
        ...
        if user_info.part == 0:
            if event.message.text == "我到資工系系館了" or event.message.text == "我到德田館了":
                reply = [] #一次可傳多對話，至多五句
                reply.append(TextSendMessage("我：我發現德田館外面的地上掉了一張紙，我觀察了一下，發現紙上的格子對應著資工系館的窗戶格子。"))
                #資工系謎題圖片
                line_bot_api.reply_message(event.reply_token, reply)
                User_Info.objects.filter(uid=event.source.user_id).update(part=1)
            else:
            ...
            if event.message.text == "KC":
                ...
                #日記照片
                reply.append(TextSendMessage("大笨鳥的靈魂：那麼接下來去註冊組問問看，誰是這張學生證的主人吧！"))
    ...
    if isinstance(event, MessageEvent):
        ...
        if user_info.part == 0:
            ...
            if event.message.text == "150" or event.message.text == "150度":
                ...
                #打卡畫面
                reply.append(TextSendMessage("大笨鳥的靈魂：他在醉月湖！我們快追上去！"))
    ...
    if isinstance(event, MessageEvent):
        ...
        if user_info.part == 0:
            if event.message.text == "我到醉月湖了":
                reply = [] #一次可傳多對話，至多五句
                reply.append(TextSendMessage("大笨鳥的靈魂：你看！那台是你的腳踏車對吧?"))
                ...
                reply.append(TextSendMessage("鵝：請問你掉的腳踏車是哪一台腳踏車呢？「金色的」？「銀色的」？還是「被偷走的腳踏車」？"))
                line_bot_api.reply_message(event.reply_token, reply)
                User_Info.objects.filter(uid=event.source.user_id).update(part=1)
            else:
                line_bot_api.reply_message(event.reply_token, TextSendMessage("大笨鳥的靈魂：我們快去醉月湖找他!"))
        elif user_info.part == 1:
            if event.message.text == "被偷走的腳踏車":
                reply = [] #一次可傳多對話，至多五句
                reply.append(TextSendMessage("鵝：好，看在你這麼誠實的份上，我就請小動物們都一起來幫助你！但是你要先記得醉月湖的「緊急數字」才能確保這次行動的安全！"))
                line_bot_api.reply_message(event.reply_token, reply)
                User_Info.objects.filter(uid=event.source.user_id).update(part=2)
            else:
                line_bot_api.reply_message(event.reply_token, TextSendMessage("鵝: 你確定嗎？ 誠實才會有好報喔！"))
        elif user_info.part == 2:
    ...
    if isinstance(event, MessageEvent):
        ...
        if user_info.part == 0:
            if event.message.text == "我到詩牆了":
                reply = [] #一次可傳多對話，至多五句
                reply.append(TextSendMessage("我：犯人居然把我的腳踏車丟到湖裡，想要毀屍滅跡，又給我這個紙條到底在想什麼？"))
                reply.append(TextSendMessage("大笨鳥的靈魂：可能是某個讓犯人心神不寧的句子吧？"))
                line_bot_api.reply_message(event.reply_token, reply)
                User_Info.objects.filter(uid=event.source.user_id).update(part=1)
            else:
            ...
            if event.message.text == "「那時我是老人了，」我說：「然而我會永遠認得你」":
                ...
                reply.append(TextSendMessage("謝謝你陪我找到犯人了，我可以安心的成佛了！希望你跟你之後的腳踏車一生平安"))
                line_bot_api.reply_message(event.reply_token, reply)
                User_Info.objects.filter(uid=event.source.user_id).update(part=0)
